...
```bash
splat -t tx_site -r rx_site -o topo_map
```

By default, **SPLAT!** generates PNG format images. The output filename extension is optional; if omitted, **SPLAT!** will append the appropriate extension based on the selected image format (.png, .jpg, .ppm, or .tif). Use the `-ppm`, `-jpg`, or `-tif` switches to change the output format. The `-proj` switch can be used with GDAL formats (-png, -tif, or -jpg) to set the map projection (epsg:3857 for Web Mercator or epsg:4326 for WGS84).

In this example, `topo_map.png` will illustrate the locations of the transmitter and receiver sites specified. In addition, the great circle path between the two sites will be drawn over locations for which an unobstructed path exists to the transmitter at a receiving antenna height equal to that of the receiver site (specified in `rx_site.qth`).
...
**SPLAT!** generated topographic maps are 24-bit TrueColor images. By default, PNG format is used, which provides lossless compression and is highly recommended for storage. Alternative formats (PPM, JPG, or GeoTIFF) can be selected using the appropriate command-line switches. Images may be viewed, edited, or converted to other graphic formats by popular image viewing applications such as **xv**, **The GIMP**, **ImageMagick**, and **XPaint**.
...
The resulting image file can be converted to PNG format with a transparent background using **ImageMagick**'s **convert** utility:

```bash
convert -transparent "#FFFFFF" white_map.png transparent_map.png
```

The `-proj` option allows you to specify the map projection for GDAL-based output formats (PNG, JPG, or GeoTIFF). This option is only available when GDAL support is compiled into **SPLAT!**. Two projections are supported:
...
```bash
splat -t tx_site -L 30.0 -o coverage -tif -proj epsg:3857
```

Note that the `-proj` option only works with GDAL output formats. If used with PPM output, it will be ignored with a warning message.
...
```bash
splat -t wnjt-dt -c 30.0 -m 1.333 -s cities.dat -b counties.dat -o map
```
...
```bash
splat -t wnjt-dt -L 30.0 -s cities.dat -b co34_d00.dat -db 140 -o plot
```
...
```bash
splat -t wnjt-dt -L 30.0 -erp 46000 -db 30 -o plot
```
...
```bash
splat -t wnjt-dt -L 30.0 -erp 46000 -dbm -db -100 -o plot
```
...
Comments can be placed in the file if they are preceeded by a semicolon. The **vim** text editor has proven capable of editing files of this size.
...
```bash
splat -t kvea -ani pathloss.dat -s city.dat -b county.dat -o map
```
...
User-defined terrain (UDT) files allow users to add custom terrain features to the digital elevation model for modeling buildings, towers, hills, or other structures not present in the base SDF terrain data. These files are particularly useful for urban environments, radio infrastructure, and known obstacles in the signal path.

### File Format

A user-defined terrain file (`.udt`) is a CSV-formatted text file containing latitudes, longitudes, and heights above ground level of specific terrain features. The file has the following structure:

```
; Comments allowed (lines starting with semicolon)
...
3. **Height**: Height of terrain feature above ground level
   ...
   - **Metric units**: Add `m`, `M`, or `meters` after the number
   - Examples: `100.0`, `30.5 m`, `150 M`, `100.0 meters`
   - Only positive values are added to the terrain model

### Usage

A user-defined terrain file is imported into a **SPLAT!** analysis using the `-udt` switch:

```bash
splat -t tx_site -r rx_site -udt buildings.udt -o map
```

Multiple UDT files can be loaded by specifying the `-udt` switch multiple times:

```bash
splat -t tx_site -r rx_site -udt buildings.udt -udt towers.udt -o map
```
...
- **Comments**: Lines starting with `;` are ignored, as is text after `;` on any line
- **Coordinate formats**: Both decimal degrees and DMS (degrees minutes seconds) are supported
- **Unit conversion**: Heights without units are assumed to be in feet; heights with 'M', 'm', or 'meters' are treated as meters
- **Duplicate handling**: Multiple entries at the same coordinate are merged, with the maximum height being used
- **Resolution**: Features are interpreted as being 3 arc-seconds in latitude and longitude in standard mode, and 1 arc-second in HD mode
- **Integration**: UDT features are added *on top of* existing SDF terrain elevations, not replacing them

### Use Cases

- **Urban environments**: Model buildings, skyscrapers, and urban clutter
- **Radio infrastructure**: Add towers, antennas, and other tall structures
- **Known obstructions**: Model specific obstacles in the signal path
- **Terrain corrections**: Supplement low-resolution DEM data with known features
- **Custom scenarios**: Test propagation with planned or temporary structures

### Important Notes

- Features are added to the base terrain elevation from SDF files
- Zero or negative heights are ignored
- Coordinates must fall within the loaded terrain region
- Resolution is limited by the SDF data resolution (standard: 3 arc-seconds, HD: 1 arc-second)
...
```bash
splat -t tx_site -r rx_site -n -o topo_map
```
...
```bash
splat -t tx_site -R 50.0 -s NJ_Cities -b NJ_Counties -o topo_map
```
...
If the `-o` switch and output filename are omitted in these operations, topographic output is written to a file named `tx_site.png` in the current working directory by default.
...
```bash
splat -t kd2bd -R 50.0 -s NJ_Cities -b NJ_Counties -geo -o map
```

The georeference file generated will have the same base name as the `-o` file specified, but have a `.geo` extension, and permit proper interpretation and display of **SPLAT!**'s graphics in **Xastir** software.
