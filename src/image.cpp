...
#ifndef _WIN32
#define RGB(signal, r, g, b)                                                   \
    (((uint32_t) (uint8_t) signal) | ((uint32_t) ((uint8_t) r) << 8) |         \
     ((uint32_t) ((uint8_t) g) << 16) | ((uint32_t) ((uint8_t) b) << 24))
#endif
...
Pixel Image::GetPixel(const Dem *dem, MapType maptype, Region &region, int x0,
                      int y0) {
    ...
    if (maptype != MAPTYPE_PATHLOSS) {
        ...
        if (match < region.levels) {
            if (sr.smooth_contours && match > 0) {
                red = (unsigned) Utilities::interpolate(
                    region.color[match][0], region.color[match - 1][0],
                    region.level[match], region.level[match - 1], signal);
                green = (unsigned) Utilities::interpolate(
                    region.color[match][1], region.color[match - 1][1],
                    region.level[match], region.level[match - 1], signal);
                blue = (unsigned) Utilities::interpolate(
                    region.color[match][2], region.color[match - 1][2],
                    region.level[match], region.level[match - 1], signal);
            } else {
        ...
        if (match < region.levels) {
            if (sr.smooth_contours && match > 0) {
                red = (unsigned) Utilities::interpolate(
                    region.color[match - 1][0], region.color[match][0],
                    region.level[match - 1], region.level[match], signal);
                green = (unsigned) Utilities::interpolate(
                    region.color[match - 1][1], region.color[match][1],
                    region.level[match - 1], region.level[match], signal);
                blue = (unsigned) Utilities::interpolate(
                    region.color[match - 1][2], region.color[match][2],
                    region.level[match - 1], region.level[match], signal);
            } else {
                red = region.color[match][0];
                green = region.color[match][1];
    ...
    if (mask & 2) {
        ...
        case MAPTYPE_PATHLOSS:
            ...
            if (signal == 0 || (sr.contour_threshold != 0 &&
                                signal > abs(sr.contour_threshold))) {
                if (sr.ngs) {
                    /* No terrain */
                    ...
                    if (dem->data[x0 * sr.ippd + y0] == 0) {
                        ...
                        terrain =
                            (unsigned) (0.5 +
                                        pow((double) (dem->data[x0 * sr.ippd +
                                                                y0] -
                                                      em.min_elevation),
                                            one_over_gamma) *
                                            conversion);
...
int Image::GetIndexForColorKeyImageFile(MapType maptype, Region &region, int x0,
                                        int y0) {
...
void Image::WriteColorKeyImageFile(const std::string &ckfile,
    ...
    try {
        ImageWriter iw =
            ImageWriter(ckfile, imagetype, width, height, 0, 0, 0, 0);

        for (int y0 = 0; y0 < (int) height; y0++) {
            for (int x0 = 0; x0 < (int) width; x0++) {
                int indx = GetIndexForColorKeyImageFile(maptype, region, x0,
                                                        y0);  // end pathloss
...
int Image::GetIndexForLegend(int colorwidth, MapType maptype, Region &region,
                             int x0, int y0) {
    int level, hundreds, tens, units;
...
void Image::WriteCoverageMap(MapType maptype, ImageType imagetype,
                             Region &region) {
    std::string basename, mapfile, geofile, kmlfile, ckfile, suffix,
        description;
#if DO_KMZ
    std::string kmzfile;
#endif
    ...
    width = (unsigned) (sr.ippd *
                        Utilities::ReduceAngle(em.max_west - em.min_west));
    ...
    if (sr.kml && (sr.geo == 0)) {
        WriteKmlForImage("SPLAT! " + description + " Contours",
                         xmtr[0].name + " Transmitter Contours",
                         sr.bottom_legend, kmlfile, mapfile, north, south, east,
                         west, ckfile);
    }

    fd = fopen(mapfile.c_str(), "wb");

    std::cout << "Writing " << description << " map \"" << mapfile << "\" ("
              << imgwidth << "x" << imgheight << " image)...\n";
...
void Image::WriteGeo(const std::string &geofile, const std::string &mapfile,
