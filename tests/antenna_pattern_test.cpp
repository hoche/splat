...
class AntennaPatternTest : public ::testing::Test {
  protected:
    ...
    std::vector<float>
        elevation_pattern;  // Simulated storage for .el file data
    MockFileIO mock_file_io;
};

// Mock function to simulate ReadAntennaPattern (adjust signature as needed)
bool ReadAntennaPattern(const std::string &filename,
                        std::vector<float> &pattern, MockFileIO &file_io) {
    std::vector<std::string> lines;
...
TEST_F(AntennaPatternTest, ParseValidAzimuthPattern) {
    ...
    std::vector<std::string> mock_lines = {
        "0.0", "1.5", "3.2", "0.0"};  // Includes zero for perfect blocking
    ...
    ASSERT_EQ(azimuth_pattern.size(), 4)
        << "Azimuth pattern should have 4 values";
    EXPECT_FLOAT_EQ(azimuth_pattern[0], 0.0f)
        << "First value should be 0.0 (perfect blocking)";
...
TEST_F(AntennaPatternTest, ParseValidElevationPattern) {
    ...
    std::vector<std::string> mock_lines = {
        "2.0", "-1.0", "0.0"};  // Includes zero and negative value
    ...
    // Call the antenna pattern parsing function
    bool result =
        ReadAntennaPattern("test.el", elevation_pattern, mock_file_io);
    EXPECT_TRUE(result) << "Parsing valid elevation pattern should succeed";
    ASSERT_EQ(elevation_pattern.size(), 3)
        << "Elevation pattern should have 3 values";
    EXPECT_FLOAT_EQ(elevation_pattern[0], 2.0f) << "First value should be 2.0";
    EXPECT_FLOAT_EQ(elevation_pattern[1], -1.0f)
        << "Second value should be -1.0";
    EXPECT_FLOAT_EQ(elevation_pattern[2], 0.0f)
        << "Third value should be 0.0 (perfect blocking)";
}
...
TEST_F(AntennaPatternTest, ParseInvalidData) {
    ...
    // Call the antenna pattern parsing function
    bool result =
        ReadAntennaPattern("invalid.az", azimuth_pattern, mock_file_io);
    EXPECT_FALSE(result) << "Parsing file with invalid data should fail";
    EXPECT_TRUE(azimuth_pattern.empty())
        << "Azimuth pattern should remain empty on failure";
}
...
TEST_F(AntennaPatternTest, FileReadFailure) {
    ...
    // Call the antenna pattern parsing function
    bool result =
        ReadAntennaPattern("missing.az", azimuth_pattern, mock_file_io);
...
class AntennaPatternSizeTest : public AntennaPatternTest,
...
TEST_P(AntennaPatternSizeTest, ParseVariousSizes) {
    ...
    // Call the antenna pattern parsing function
    bool result = ReadAntennaPattern("test.az", azimuth_pattern, mock_file_io);
    EXPECT_TRUE(result) << "Parsing pattern of size " << GetParam()
                        << " should succeed";
    EXPECT_EQ(azimuth_pattern.size(), static_cast<size_t>(GetParam()))
        << "Pattern size should match input";
...
INSTANTIATE_TEST_SUITE_P(
    PatternSizes, AntennaPatternSizeTest,
    ::testing::Values(
        0, 1, 10,
        360));  // Empty, single value, small pattern, full 360-degree pattern
