...
class MockFileIO {
  public:
    MOCK_METHOD2(read_file, bool(const std::string &filename,
...
class InputFileParsingTest : public ::testing::Test {
  protected:
    ...
    std::vector<std::vector<float>>
        sdf_data;  // Simulated storage for SDF elevation data
    double epsilon;
...
bool ParseSDF(const std::string &filename,
              std::vector<std::vector<float>> &elevation_data,
              MockFileIO &file_io) {
    std::vector<std::string> lines;
...
TEST_F(InputFileParsingTest, ParseValidSDF) {
    ...
    std::vector<std::string> mock_lines = {
        "100.0 150.0 200.0", "120.0 130.0 140.0", "110.0 160.0 180.0"};
    EXPECT_CALL(mock_file_io, read_file("46_47_122_123.sdf", testing::_))
        .WillOnce(testing::DoAll(testing::SetArgReferee<1>(mock_lines),
                                 testing::Return(true)));
    ...
    EXPECT_NEAR(sdf_data[0][0], 100.0, epsilon)
        << "First row, first value should be 100.0";
    EXPECT_NEAR(sdf_data[1][1], 130.0, epsilon)
        << "Second row, second value should be 130.0";
    EXPECT_NEAR(sdf_data[2][2], 180.0, epsilon)
        << "Third row, third value should be 180.0";
}
...
TEST_F(InputFileParsingTest, ParseInvalidSDF) {
    ...
    std::vector<std::string> mock_lines = {
        "100.0 150.0 200.0", "120.0 invalid 140.0", "110.0 160.0 180.0"};
...
TEST_F(InputFileParsingTest, ParseInconsistentSDF) {
    ...
    std::vector<std::string> mock_lines = {"100.0 150.0 200.0", "120.0 130.0",
                                           "110.0 160.0 180.0"};
    ...
    EXPECT_TRUE(result) << "Parsing SDF with inconsistent rows should succeed "
                           "but store valid rows";
    ASSERT_EQ(sdf_data.size(), 3) << "SDF data should have 3 rows";
    EXPECT_EQ(sdf_data[1].size(), 2) << "Second row should have 2 columns";
    EXPECT_NEAR(sdf_data[1][0], 120.0, epsilon)
        << "Second row, first value should be 120.0";
    EXPECT_NEAR(sdf_data[1][1], 130.0, epsilon)
        << "Second row, second value should be 130.0";
}
...
TEST_F(InputFileParsingTest, ParseSDFWithWhitespace) {
    ...
    std::vector<std::string> mock_lines = {"  100.0   150.0  200.0  ",
                                           "\t120.0 130.0\t140.0",
                                           "110.0\t 160.0  180.0 "};
    ...
    EXPECT_TRUE(result)
        << "Parsing SDF with whitespace variations should succeed";
    ASSERT_EQ(sdf_data.size(), 3) << "SDF data should have 3 rows";
    ASSERT_EQ(sdf_data[0].size(), 3) << "Each row should have 3 columns";
    EXPECT_NEAR(sdf_data[0][0], 100.0, epsilon)
        << "First row, first value should be 100.0";
...
class SDFGridSizeTest
    : public InputFileParsingTest,
...
TEST_P(SDFGridSizeTest, ParseVariousGridSizes) {
    ...
    EXPECT_TRUE(result) << "Parsing SDF with grid size "
                        << std::get<0>(GetParam()) << "x"
                        << std::get<1>(GetParam()) << " should succeed";
    EXPECT_EQ(sdf_data.size(), static_cast<size_t>(std::get<0>(GetParam())))
        << "SDF data should have correct number of rows";
    if (! sdf_data.empty()) {
        EXPECT_EQ(sdf_data[0].size(),
                  static_cast<size_t>(std::get<1>(GetParam())))
            << "SDF data should have correct number of columns";
        for (size_t i = 0; i < sdf_data.size(); ++i) {
            for (size_t j = 0; j < sdf_data[i].size(); ++j) {
                EXPECT_NEAR(sdf_data[i][j],
                            100.0 + i * std::get<1>(GetParam()) + j, epsilon)
                    << "Value at (" << i << "," << j
