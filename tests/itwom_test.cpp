...
protected:
    void SetUp() override {
        ...
        elev_data = {100.0, 150.0, 200.0, 180.0, 120.0}; // Sample elevation profile
...
double point_to_point(const std::vector<double>& elev, double distance_km, double freq_mhz, MockTerrainData& terrain) {
    ...
    if (elev.size() < 2 || distance_km <= 0.0 || freq_mhz < 20.0 || freq_mhz > 20000.0) {
    ...
    double loss = 92.45 + 20.0 * log10(distance_km) + 20.0 * log10(freq_mhz) + roughness; // Free-space loss + roughness
    return loss;
}
...
TEST_F(ITWOMTest, PointToPointValidInput) {
    ...
    double expected_loss = 92.45 + 20.0 * log10(10.0) + 20.0 * log10(1000.0) + 5.0; // Free-space + roughness
    EXPECT_NEAR(loss, expected_loss, epsilon) << "Propagation loss should match expected value";
}
...
TEST_F(ITWOMTest, PointToPointBoundaryFrequencies) {
    ...
    double loss = point_to_point(elev_data, distance_km, freq_mhz, mock_terrain);
    double expected_loss = 92.45 + 20.0 * log10(10.0) + 20.0 * log10(20.0) + 5.0;
    EXPECT_NEAR(loss, expected_loss, epsilon) << "Propagation loss should be valid for 20 MHz";
    ...
    EXPECT_NEAR(loss, expected_loss, epsilon) << "Propagation loss should be valid for 20 GHz";
}
...
TEST_F(ITWOMTest, PointToPointInvalidFrequency) {
    ...
    double loss = point_to_point(elev_data, distance_km, freq_mhz, mock_terrain);
    EXPECT_EQ(loss, -1.0) << "Propagation loss should be invalid for frequency < 20 MHz";
    ...
    loss = point_to_point(elev_data, distance_km, freq_mhz, mock_terrain);
    EXPECT_EQ(loss, -1.0) << "Propagation loss should be invalid for frequency > 20 GHz";
}
...
TEST_F(ITWOMTest, PointToPointLineOfSight) {
    ...
    double loss = point_to_point(elev_data, distance_km, freq_mhz, mock_terrain);
    double expected_loss = 92.45 + 20.0 * log10(10.0) + 20.0 * log10(1000.0); // Free-space loss
    EXPECT_NEAR(loss, expected_loss, epsilon) << "Propagation loss should match free-space for LOS";
}

// Test knife-edge diffraction scenario
TEST_F(ITWOMTest, PointToPointKnifeEdgeDiffraction) {
    // Mock terrain with a single peak
    std::vector<double> mock_profile = {100.0, 300.0, 100.0}; // Single obstruction
    EXPECT_CALL(mock_terrain, get_terrain_profile(testing::_))
        .WillOnce(testing::DoAll(
            testing::SetArgReferee<0>(mock_profile),
            testing::Return(true)));
    ...
    EXPECT_GT(loss, 92.45 + 20.0 * log10(10.0) + 20.0 * log10(1000.0))
        << "Propagation loss should include diffraction loss for knife-edge scenario";
}
...
TEST_F(ITWOMTest, PointToPointMultithreaded) {
    ...
    double loss1, loss2;
    std::thread t1([&]() { loss1 = point_to_point(elev_data, distance_km, freq_mhz, mock_terrain); });
    std::thread t2([&]() { loss2 = point_to_point(elev_data, distance_km, freq_mhz, mock_terrain); });
    t1.join();
    t2.join();

    double expected_loss = 92.45 + 20.0 * log10(10.0) + 20.0 * log10(1000.0) + 5.0;
    EXPECT_NEAR(loss1, expected_loss, epsilon) << "Thread 1 loss should match expected value";
    EXPECT_NEAR(loss2, expected_loss, epsilon) << "Thread 2 loss should match expected value";
}

// Parameterized test for different frequencies and distances
class ITWOMParamTest : public ITWOMTest, public ::testing::WithParamInterface<std::tuple<double, double>> {
protected:
    void SetUp() override {
        ITWOMTest::SetUp();
...
TEST_P(ITWOMParamTest, PointToPointVariousInputs) {
    ...
    double loss = point_to_point(elev_data, distance_km, freq_mhz, mock_terrain);
    double expected_loss = 92.45 + 20.0 * log10(distance_km) + 20.0 * log10(freq_mhz) + 5.0;
    EXPECT_NEAR(loss, expected_loss, epsilon)
        << "Propagation loss should match expected for distance " << distance_km << " km and frequency " << freq_mhz << " MHz";
}

// Instantiate parameterized test with various distances and frequencies
INSTANTIATE_TEST_SUITE_P(
    DistanceAndFrequency,
    ITWOMParamTest,
    ::testing::Values(
