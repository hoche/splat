...
char sdf_filename[30], sdf_path[255], replacement_flag, opened = 0, hgt = 0,
                                                        bil = 0;
char colons = 0;
...
int ReadSRTM(char *filename) {
    ...
    if ((strstr(filename, ".hgt") == NULL) &&
        (strstr(filename, ".bil") == NULL)) {
        fprintf(stderr,
                "*** Error: \"%s\" does not have the correct extension (.hgt "
                "or .bil)\n",
                filename);
        return -1;
    }
    ...
    if (hgt) {
        ...
        if ((base[0] != 'N' && base[0] != 'S') ||
            (base[3] != 'W' && base[3] != 'E')) {
            fprintf(stderr,
                    "*** Error: \"%s\" doesn't look like a valid .hgt SRTM "
                    "filename.\n",
                    filename);
            return -1;
        }
    ...
    if (colons) {
        sprintf(sdf_filename, "%d:%d:%d:%d%s.sdf", min_north, max_north,
                min_west, max_west, (ippd == 3600 ? "-hd" : ""));
    } else {
        sprintf(sdf_filename, "%d_%d_%d_%d%s.sdf", min_north, max_north,
                min_west, max_west, (ippd == 3600 ? "-hd" : ""));
    }
...
int ReadUSGS() {
    ...
    sprintf(usgs_filename, "%d:%d:%d:%d", min_north, max_north, min_west,
            max_west);

    return (LoadSDF(usgs_filename));
}

void average_terrain(y, x, z) int x, y, z;
{
    long accum;
...
int main(int argc, char **argv) {
    ...
    if (argc == 1 || (argc == 2 && strncmp(argv[1], "-h", 2) == 0)) {
        fprintf(stderr, "\nsrtm2sdf: Generates SPLAT! elevation data files "
                        "from unzipped\nSRTM-1 or SRTM-3 elevation data files, "
                        "and replaces SRTM data voids with\nelevation data "
                        "from older usgs2sdf derived SDF files.\n\n");

        fprintf(stderr, "\tAvailable Options...\n\n");
        fprintf(stderr, "\t-c Use colons in the filename instead of x's\n\t    "
                        "This mimics older behavior, but generates files that "
                        "cannot be used on Windows filesystems.\n\n");
        fprintf(stderr, "\t-d directory path of usgs2sdf derived SDF files\n\t "
                        "   (overrides path in ~/.splat_path file)\n\n");
        fprintf(
            stderr,
            "\t-hd Invoke in High Definition mode, working on SRTM-1 files\n\t "
            "   Without this, it expects the input files to be SRTM-3 "
            "files.\n\t    You can also invoke the binary as %s-hd\n\n",
            argv[0]);
        fprintf(
            stderr,
            "\t-n elevation limit (in meters) below which SRTM data is\n\t    "
        ...
        fprintf(stderr,
                "          %s -d /dev/null N40W074.hgt  (prevents data "
                "replacement)\n",
                argv[0]);
